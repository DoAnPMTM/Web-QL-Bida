...
namespace QL_QuanBi_a_API.Controllers
{
    ...
    public class BansController : ControllerBase
    {
        ...
        [HttpGet("GetName")]
        public async Task<ActionResult<String>> GetNameTableById([FromQuery] int idTable)
        {
            var nametable = await _context.Bans.FindAsync(idTable);
            if (nametable == null)
                return NotFound();
            return Ok(nametable.Tenban);
        }
